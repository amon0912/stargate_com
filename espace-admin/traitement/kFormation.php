...
if (!empty($_POST['titre']) && !empty($_POST['description']) && !empty($_POST['reduction']) && !empty($_POST['idTypeFor'])) {
    ...
    if ($_FILES['lienImage']['error'] != 0) {
        ...
        if (in_array($extension_upload, $extensions_autorisees)) {
            ...
            $req = $db->prepare(" INSERT INTO formation (idFormation,idTypeFor,id, titre,description,reduction,lienImage) VALUES (?,?,?,?,?,?,?) ");
            $req->execute([$id, $idTypeFor, $_SESSION['id'], $titre, $description, $reduction, $lien]);
